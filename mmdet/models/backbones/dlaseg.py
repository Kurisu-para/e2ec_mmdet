...
class DLA(nn.Module):
    ...
    def load_pretrained_model(self, data='imagenet', name='dla34', hash='ba72cf86'):
        ...
        classes = len(model_weights[list(model_weights.keys())[-1]])
        self.fc = nn.Conv2d(
            self.channels[-1], classes,
...
class DLAUp(nn.Module):
    ...
    def forward(self, layers):
        # out = [layers[-1]] # 源代码。实际上开始的layers[-1]这个变量没用到，影响多卡训练
        out = []
...
class DLASeg(nn.Module):
    ...
    def forward(self, x):
        ...
        y = []
        for i in range(self.last_level - self.first_level): # range(3)
            y.append(x[i].clone())
