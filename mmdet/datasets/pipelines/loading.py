...
class LoadAnnotations:
    ...
    def prepare_evolution(self, poly, img_gt_polys, can_gt_polys, keyPointsMask):
        img_gt_poly = self.uniformsample(poly, len(poly) * self.cfg.data.points_per_poly)
    ...
    def _load_masks(self, results):
        ...
        if self.poly2mask:
            ...
            # detection
            output_h, output_w = inp_out_hw[2:]
            ct_hm = np.zeros([self.cfg.model.heads['ct_hm'], output_h, output_w], dtype=np.float32)
            ct_cls = []
            wh = []
            ct_ind = []
